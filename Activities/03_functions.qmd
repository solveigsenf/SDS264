...
The function removes the entire row that contains the NA value.
...
R is recycling the weights 1:3 twice because there are twice as many values to be averaged than there are weights.  
...
call. determines whether or not to display the function that is causing the error. In this example, when call. = TRUE, R prints "Error in wt_mean(1:6, 1:3)" and the warning message. 
...
if(is.na(x)) prints a vector with TRUE/FALSE values for each value in the original vector, whereas is.na(sum(x)) displays the total number of values that are NAs. 
...
This code will produce a tibble that only contains certain car manufactures, counts the number of observations in the defined variable (manufacturer), sorts them numerically from most to least, and makes a column that is the number of observations of each manufacture divided by the total observations. We need sort = sort because it tells R where to place the sort (numerically by count vs alphabetically by filter). df is already an env-variable so we don't need to embrace it. We don't need to define n because it was created by the count function. 
...
This function will count the number of unique combinations of manufacturers and models. Then, it will name the columns by number of cylinders and the values will be number of observations in that unique combination of manufacturer and model Then it wil sort the names alphabetically and fill missing values with 0.
...
```{r}
proportion_sum <- function(x) {
  (x/sum(x, na.rm = TRUE))
}

proportion_sum(temp0)
proportion_sum(temp)
```
...
Our function does not handle NAs so if there is one NA observation for the carrier, the output is NA.
...
flights |>
  group_by(carrier) |>
  summarize(
    se_arr_delay = se_na(arr_delay),
    se_dep_delay = se_na(dep_delay),
...
I did not get the output I wanted because the function does not handle two vectors of different lengths. 
...
flights |>
  summarize(
    ...
    na_both = both_na(dep_delay, arr_delay)
  )
```

Yes, there are more NAs for arrival delay than there are for departure delay.
...
time_difference_min <- function(time1, time2) {
  hour1 <- time1 %/% 100
  min1 <- time1 %% 100
  hour2 <- time2 %/% 100
  min2 <- time2 %% 100
...
First the function finds the difference between values (?) and then sums the amount of times diff < 0.
...
foo1 <- function(x) {
  if(sum(is.na(x) > 0)) {
    stop("x can not have missing values", call. = FALSE)
  }
  else {
  diff <- x[-1] - x[1:(length(x) - 1)] #x[-1] = remove first element, x[1:(length(x)-1)] removes the last element depending on the length of the vector (?) 
  sum(diff < 0) 
  }
}
...
```{r}
summary6 <- function(data, var, group_vars, na.rm = TRUE) {
  data |> 
    group_by(pick({{ group_vars }})) |>
    summarize(
...
#doesn't summary6 already give the option to remove missing values with na.rm = TRUE or na.rm = FALSE

mpg_tbl <- as_tibble(mpg)

mpg_tbl |> summary6(hwy)

gss_cat |> summary6(age)

mpg_tbl |> summary6(hwy, drv)

party_age <- gss_cat |> summary6(age, partyid)
```
...
```{r}
last_value_in_vector <- function(x) {
  value <- x[(length(x))]
  print(value)
}

last_value_in_vector(x)
```
...
ggplot_func <- function(dataset, mean_median, sd_IQR, group)  {
  label <- rlang::englue("A scatterplot of {{mean_median}} vs {{sd_IQR}}")
  
  group_count <- dataset |> group_by({{ group }}) |> mutate(n = n()) |> ungroup()
  
  ggplot(data = dataset, mapping = aes(x = {{ mean_median }}, y = {{ sd_IQR }})) +
...
ggplot_func(party_age, mean, sd, partyid)
