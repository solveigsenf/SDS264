...
```{r}
bind_cols(mpls_data1, mpls_data2) |> #merging the two tables
  as_tibble()  |> #turns the table into a tibble
  select(-`...8`) |> #getting rid of the second name column
  mutate(`...1` = str_extract(`...1`, "[^ ]+ [^ ]+ [^ ]+")) |> #keep the first three words in the "...1" column?
  pivot_longer(cols = c(`JanJa`:`DecDe`), 
               names_to = "month", values_to = "weather") |> #turns the month columns into a single "month" column, and turns the values for weather indicators into observations of a "weather" column.
  pivot_wider(names_from = `...1`, values_from = weather) |> #takes the values from the "...1" column and turns them into their own columns
  mutate(month = str_sub(month, 1, 3))  |> #keeps the first three characters in the month observations
  rename(avg_high = "Average high in",
         avg_low = "Average low in") #renames the average high and low columns
...
```{r}
pub_pri <- mn_colleges$public_private
```
