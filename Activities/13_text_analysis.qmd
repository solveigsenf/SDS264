...
bing_potter <- potter_tidy |>
  ...
  group_by(sentiment) |>
  slice_max(n, n = 10) |>
  mutate(color = sentiment, c("negative" = "red", "positive" = "yellow"))
...
potter_names_long <- potter_names |>
  pivot_longer(cols = ends_with("tname"),
               names_to = "type",
               values_to = "name")

potter_section_words <- potter_untidy |>
  select(-book_num, -chapter) |>
  mutate(section = row_number() %/% 80) |> 
  filter(section > 0) |>
  unnest_tokens(word, text) |> 
  filter(!word %in% stop_words$word,
         !is.na(word)) |>
  anti_join(potter_spells, by = c("word" = "spell")) |>
  anti_join(potter_names_long, by = c("word" = "name"))
